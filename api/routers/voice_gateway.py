...
async def update_file(file_id: str, request: Request):
    ...
    try:
        ...
        if action == 'retranscribe':
            ...
            # 热词从 config.py 读取，不再从前端传递
            hotword = MODEL_CONFIG.get('hotword', '')
...
# 热词存储文件路径
HOTWORD_STORAGE_FILE = os.path.join(FILE_CONFIG.get('output_dir', 'transcripts'), '.hotwords.json')

def _load_hotwords() -> dict:
    """加载保存的热词"""
    try:
        if os.path.exists(HOTWORD_STORAGE_FILE):
            with open(HOTWORD_STORAGE_FILE, 'r', encoding='utf-8') as f:
                return json.load(f)
    except Exception as e:
...
def _save_hotwords(data: dict):
    """保存热词"""
    try:
        os.makedirs(os.path.dirname(HOTWORD_STORAGE_FILE), exist_ok=True)
        with open(HOTWORD_STORAGE_FILE, 'w', encoding='utf-8') as f:
            json.dump(data, f, ensure_ascii=False, indent=2)
    except Exception as e:
        logger.error(f"保存热词失败: {e}")
        raise HTTPException(status_code=500, detail=f"保存热词失败: {str(e)}")
...
async def hotwords_operation(request: Request):
    """
    统一的热词操作接口
    
    通过请求体中的 action 参数区分不同操作：
    - preset: 保存/更新热词预设
    - validate: 验证热词格式
    - history: 添加热词使用历史
    ...
    try:
        ...
        if not action:
            raise HTTPException(status_code=400, detail="缺少 action 参数，支持的操作: preset, validate, history")
        
        if action == 'preset':
            # 保存热词预设
            name = body.get('name', '').strip()
            hotwords = body.get('hotwords', '').strip()  # 允许为空，默认为空字符串
            ...
            processed_hotwords = text_processor.process_hotword(hotwords)
            stats = text_processor.get_hotword_statistics(processed_hotwords)
            ...
            preset = {
                'name': name,
                'hotwords': processed_hotwords,
                'description': description,
                'stats': stats,
                'created_at': datetime.now().isoformat() if existing_index is None else presets[existing_index].get('created_at', datetime.now().isoformat()),
            ...
            # 验证热词格式（允许为空）
            hotwords = body.get('hotwords', '').strip()  # 默认为空字符串
            
            text_processor = TextProcessor()
            processed = text_processor.process_hotword(hotwords)
            stats = text_processor.get_hotword_statistics(processed)
            
            # 检查错误（空热词是有效的，不报错）
            errors = []
            if hotwords:  # 只有当热词不为空时才进行验证
                if stats['count'] == 0:
                    errors.append("没有有效的热词")
                elif stats['count'] > text_processor.MAX_HOTWORDS_COUNT:
                    errors.append(f"热词数量过多（最多{text_processor.MAX_HOTWORDS_COUNT}个）")
            ...
            # 去重：如果最近使用过相同的热词，更新时间
            existing_index = next((i for i, h in enumerate(history) if h.get('hotwords') == hotwords), None)
            if existing_index is not None:
                history[existing_index] = history_item
            else:
            ...
            raise HTTPException(status_code=400, detail=f"不支持的操作: {action}，支持的操作: preset, validate, history")
    
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"热词操作失败: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@router.delete("/hotwords")
async def delete_hotword_preset(name: str = None):
    """
    删除热词预设
